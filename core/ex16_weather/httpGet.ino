...
// 注意 out_lenはバッファサイズではなく、応答最大文字サイズ
    ...
    switch(data_number){
        ...
        case 5:
            ...
            break;
        case 6:
            snprintf(out,out_len+1,"%s %d | %d",_ht_data.disp,(int)_ht_data.temps[0][1],(int)_ht_data.temps[1][1]);
            break;
        default:
            snprintf(out,out_len+1,"%s %d %%",_ht_data.disp,(int)_ht_data.pops[0][1]);
            break;
    }
    #ifdef DEBUG_HTTPG
...
//  size=strlen(s);
    ...
    while(https.connected() && done_prev != done){
        ...
        while(cp - s < BUF_N/2){
            ...
            if(done == target){
                http_debug_step(done,time);
                for(i=0;i<4;i++) _ht_data.temps[i][0] = -128;
                cp = _search(cp, "\"timeDefines\"",1);
                if(!cp) break;
                cp2 =_search(cp, "]");
                if(!cp2) cp2 = s + BUF_N;
                ...
                for(i=0;i<4;i++){
                    ...
                    _ht_data.temps[i][0] = atoi(cp) + 24 * day;
                    Serial.print(_ht_data.temps[i][0] % 24);
                    if(i<3)Serial.print(", ");
                }
                Serial.println();
                done++;
            ...
            if(done == target){
                http_debug_step(done,time);
                for(i=0;i<4;i++) _ht_data.temps[i][1] = -128;
                ...
                for(i=0;i<4;i++){
                    ...
                    _ht_data.temps[i][1] = atoi(cp);
                    // Serial.printf("DEBUG cp='%s'\n",cp);
                ...
                for(i=0;i<3;i++){   // Sort 
                    if(_ht_data.temps[i][0] > _ht_data.temps[i+1][0]){
                        ...
                        tmp = _ht_data.temps[i][1];
                        _ht_data.temps[i][1] = _ht_data.temps[i+1][1];
                        _ht_data.temps[i+1][1] = tmp;
                    }
                }
                done++;
            }
            ...
            if(done == target){
                ...
                if(cp){
                    ...
                    if(i != _ht_data.day || _ht_data.hour != h) Serial.println("WARN: parent's date not meet child's");
                    done++;
                }
            }
            if(done_prev < 0 && (!cp || cp - s >= BUF_N/2)) break;
...
/*
    以下はYahoo!天気・災害のRSS配信情報(配信終了)を使用する場合の処理部でした。
    Yahoo!天気・災害のRSS配信サービスは、すでに終了しています。
*/

// ご注意：Yahoo!天気・災害のRSS配信サービスは、すでに終了しています。
