...
float getTemp(){
    ...
    Wire.write(0xFD); // measure T & RH with high precision (high repeatability)
 // Wire.write(0xE0); // low repeatability
    if( Wire.endTransmission() == 0){
        ...
        if(Wire.available()==0) return -999.;
        crc[1] = Wire.read();
        Serial.printf("debug temp=0x%04X(%02X) hum=0x%04X(%02X)\n", temp,crc[0],hum,crc[1]);
