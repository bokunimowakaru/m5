...
void setup(){                                   // 起動時に一度だけ実行する関数
    ...
    disp("daruma3","GAME START n=" + String(prisoners)); // ゲーム開始
...
void loop(){                                    // 繰り返し実行する関数
    ...
    for(int i=0; i <= 100; i++){                // 棒グラフを増加させる
        ...
        if(M5.BtnA.isPressed()){                // Aボタンが押されたとき
            ...
            prisoners=getPrisoners(-prisoners); // 捕まった人数をサーバから減算
            disp("daruma2","Cleared! n=" + String(prisoners));         // 顔を表示
            end();                              // 終了関数endを実行
            return;                             // loop関数の先頭に戻る
        }
        if(i==33){                              // 棒グラフ33%のとき
    ...
    for(int i=100; i >= 0; i--){                // 棒グラフを減らす処理
        ...
        pir = digitalRead(PIN_PIR);             // 人感センサ値を取得
        if(pir){                                // センサ反応時
            disp("daruma2","Failed" + String(prisoners)); // "Failed"を表示
