/*******************************************************************************
M5StickC Plus用 画面キャプチャ・ツール ※M5.Lcd.readRectRGBが動作しない
********************************************************************************
...
#define BitImageSize  (240 * 135 *3)  //画面キャプチャーイメージサイズ
#define PixelPerMeter (240 / 1 * 100) //画面解像度
...
void getBmpHeader(uint8_t *FrameBuffer){    // 54バイト
    const uint8_t BMP_header[14 + 40] =  //BMPファイルヘッダー定義
      { //File header 14 bytes
        ...
        (240 & 0xff),((240 >> 8)& 0xff),0,0,  //int32_t biWidth
        135,0,0,0,                            //int32_t biHeight
...
void getBmpLine(uint8_t *FrameBuffer, int y){    // 240×3バイト
    M5.Lcd.readRectRGB(0, y, 240, 1, FrameBuffer);  //１ライン分の画面データの取得
    int sum = 0;
    uint8_t swap;
    for(int i; i<240*3;i+=3){
