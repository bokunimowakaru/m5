...
void analogMeterInit()
{
  ...
  for (int i = -50; i < 51; i += 5) {
    ...
    if (i % 25 == 0) {
      ...
      switch (i / 25) {
        case -2:
        	M5.Lcd.drawCentreString(String(analogMeterMinVal1), x0, y0 - 4, 2);
        	break;
        case -1:
        	M5.Lcd.drawCentreString(String((analogMeterMinVal1*3+analogMeterMaxVal1)/4), x0, y0 - 2, 2);
        	break;
        case 0:
        	M5.Lcd.drawCentreString(String((analogMeterMinVal1+analogMeterMaxVal1)/2), x0, y0 , 2);
        	break;
        case 1:
        	M5.Lcd.drawCentreString(String((analogMeterMinVal1+analogMeterMaxVal1*3)/4), x0, y0 - 2, 2);
        	break;
        case 2:
        	M5.Lcd.drawCentreString(String(analogMeterMaxVal1), x0, y0 - 4, 2);
        	break;
      }
    }
  ...
  M5.Lcd.drawRightString(analogMeterUnit1, M_SIZE*236, M_SIZE*116, 2); // Units at bottom right
  M5.Lcd.drawCentreString(analogMeterName1, M_SIZE*120, M_SIZE*70, 4); // Comment out to avoid font 4
...
void analogMeterNeedle(float value_fn, byte ms_delay)
{
  ...
  M5.Lcd.fillRect(3+ch*160, M_SIZE * 116, M_SIZE * 64, 16, TFT_WHITE);
  M5.Lcd.drawString(buf, M_SIZE*10+ch*160, M_SIZE*(116), 2);
  ...
  while (!(value == old_analog)) {
    ...
    M5.Lcd.drawLine(M_SIZE*(120 + 20 * ltx - 1) + ch * 160, M_SIZE*(155 - 20), osx - 1 + ch * 160, osy+1, TFT_WHITE);
    M5.Lcd.drawLine(M_SIZE*(120 + 20 * ltx) + ch * 160, M_SIZE*(155 - 20), osx + ch * 160, osy, TFT_WHITE);
    M5.Lcd.drawLine(M_SIZE*(120 + 20 * ltx + 1) + ch * 160, M_SIZE*(155 - 20), osx + 1 + ch * 160, osy+1, TFT_WHITE);

    // Re-plot text under needle
    M5.Lcd.setTextColor(TFT_BLACK);
    M5.Lcd.fillRect(M_SIZE*70 + ch * 160, M_SIZE*70, M_SIZE*100, 16,TFT_WHITE);
    M5.Lcd.drawCentreString(analogMeterName, M_SIZE*120 + ch * 160, M_SIZE*70, 4); // // Comment out to avoid font 4
    ...
    M5.Lcd.drawLine(M_SIZE*(120 + 20 * ltx - 1) + ch * 160, M_SIZE*(155 - 20), osx - 1 + ch * 160, osy+1, TFT_RED);
    M5.Lcd.drawLine(M_SIZE*(120 + 20 * ltx) + ch * 160, M_SIZE*(155 - 20), osx + ch * 160, osy, TFT_MAGENTA);
    M5.Lcd.drawLine(M_SIZE*(120 + 20 * ltx + 1) + ch * 160, M_SIZE*(155 - 20), osx + 1 + ch * 160, osy+1, TFT_RED);
