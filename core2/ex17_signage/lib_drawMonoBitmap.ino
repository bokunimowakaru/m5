...
int drawMonoBitmapFile(char *filename,int PIX_X, int PIX_Y, int white){
	////////////// 【動作未確認】 ///////////////////////
	File file;
	int i,x,y;
	int PIX_SIZE_BYTE = PIX_X * PIX_Y;
	unsigned char data[PIX_SIZE_BYTE + 0x3E + 1];
	unsigned char c;
	char s[65];
...
//	char s[]="XXX/n.txt";
	...
	Serial.printf("-- Entered drawMonoBitmapFile 【動作未確認】\n");
	Serial.printf("開始(%s)\n",filename);
//	Serial.printf("PIX_WHITE = %02x\n",PIX_WHITE);
	Serial.printf("入力ファイル確認(%s)\n",filename);
	file = SPIFFS.open(filename, "r");
	int len = file.available();
	if(len == 0) err("cannot open file; len = 0");
//	if(len > PIX_SIZE_BYTE) err("cannot open file");
//	file.seek(0x0000003E,SeekSet);
	file.read(data, PIX_SIZE_BYTE + 0x3E);
	file.close();
	Serial.printf("-- Finished drawMonoBitmapFile\n");
	return drawMonoBitmap(data, PIX_X, PIX_Y, white);
}

int drawMonoBitmapFile(char *filename){
	return drawMonoBitmapFile(filename, 320, 240, 128);
}

int drawMonoBitmapFile(char *filename, int white){
